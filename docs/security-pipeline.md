...
6. `container_scan` - сканирование опубликованного Docker image через `trivy` напрямую из GitLab Container Registry.
7. `zap_scan` - кастомный OWASP ZAP baseline scan против запущенного API.
...
- Стадия называется `zap_scan`, а не `dast`, чтобы не конфликтовать с зарезервированной GitLab DAST-конфигурацией на планах без Ultimate.
